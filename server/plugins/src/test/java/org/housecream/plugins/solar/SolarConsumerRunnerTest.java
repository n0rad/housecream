...
public class SolarConsumerRunnerTest {
    ...
    public void should_give_current_state_at_start() throws Exception {
        ...
        Mockito.when(calculator.findCurrentState()).thenReturn(new SolarState(0, 0, current, SolarTimePhase.AFTERNOON));

        Date nextExecution = runner.nextExecutionTime(triggerContext);

        Assertions.assertThat(nextExecution.getTime()).isEqualTo(current.getMillis());
    }
    ...
    public void should_give_next_event() throws Exception {
        ...
        Mockito.when(calculator.findNextEvent()).thenReturn(new SolarState(0, 0, next, SolarTimeEvent.noon));
        triggerContext.update(new Date(), new Date(), new Date());

        Date nextExecution = runner.nextExecutionTime(triggerContext);

        Assertions.assertThat(nextExecution.getTime()).isEqualTo(next.getMillis());
    }
    ...
    public void should_give_phase_state_1_min_after_event() throws Exception {
        ...
        Assertions.assertThat(nextExecution.getTime()).isEqualTo(next.plusMinutes(1).getMillis());
    }
    ...
    public void should_create_camel_exchange_with_next_event() throws Exception {
        ...
        Mockito.when(endpoint.createExchange(ExchangePattern.InOnly)).thenReturn(exchange);

        runner.run();

        Assertions.assertThat(((SolarState) exchange.getIn().getBody()).getPhase()).isEqualTo(SolarTimePhase.AFTERNOON);
        Assertions.assertThat(((SolarState) exchange.getIn().getBody()).getDate()).isNotNull();
    }
}
