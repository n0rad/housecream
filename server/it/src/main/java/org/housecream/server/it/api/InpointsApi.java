...
import org.housecream.server.api.resource.InPointsResource.InPointResource;
import org.housecream.server.it.ItSession;
import fr.norad.core.lang.exception.NotFoundException;

public class InpointsApi {

    private final ItSession session;

    public InpointsApi(ItSession session) {
    ...
    public InPoint getPoint(UUID pointId) throws NotFoundException {
        return session.getServer().getResource(InPointsResource.class, session).inPoint(pointId).getInPoint(pointId);
    }

    public InPointResource internalInpointResource(UUID inPointId) {
        return session.getServer().getResource(InPointsResource.class, session).inPoint(inPointId);
    }
}
