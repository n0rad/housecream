...
public class OutpointsApi {

    private final ItSession session;

    public OutpointsApi(ItSession session) {
    ...
    public void setValue(OutPoint outpoint, float value) throws Exception {
        session.getServer().getResource(OutPointsResource.class, session).outPoint(outpoint.getId()).setValue(outpoint.getId(), value);
    }
}
