...
// Structure for the configuration in flash
struct flashconfig {
    u8_t dhcpenable;
    u8_t ipaddr[4];
    u8_t netmask[4];
    u8_t gateway[4];
    char authentication[31];
    // you can add more settings here, up to 512 bytes in total
};

// Storage for the configuration
extern struct flashconfig config;

// Location of the configuration in flash
#define FLASHCONFIG 0;

// Initialize ethernet controller, bring link up
int init_CP2200(void);

// Read from network device
...
// This is already called during ethernet controller initalisation.
void readFlashConfig(void);
  
// Write configuration into flash memory.
void writeFlashConfig(void);

// Write a byte to the CP2200 chip
void write_CP2200(int adr, char value);

// Read a byte from the CP2200 chip
...
// Write a String to the flash memory at the given adress.
// The string must be 0-512 characters long and it must start at a 512-byte boundary.
void writeFlashString(char* buffer, u16_t adr);
