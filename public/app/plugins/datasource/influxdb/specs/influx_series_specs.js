...
var influx_series_1 = require("../influx_series");
common_1.describe('when generating timeseries from influxdb response', function () {
    ...
    common_1.describe('given annotation response', function () {
        common_1.describe('with empty tagsColumn', function () {
            var options = {
                alias: '',
                annotation: {},
                series: [
            ...
            common_1.it('should multiple tags', function () {
                var series = new influx_series_1.default(options);
                var annotations = series.getAnnotations();
                common_1.expect(annotations[0].tags.length).to.be(0);
            });
        });
        common_1.describe('given annotation response', function () {
            var options = {
            ...
            common_1.it('should multiple tags', function () {
                var series = new influx_series_1.default(options);
                var annotations = series.getAnnotations();
                common_1.expect(annotations[0].tags.length).to.be(2);
                common_1.expect(annotations[0].tags[0]).to.be('America');
                common_1.expect(annotations[0].tags[1]).to.be('backend');
            });
        });
    });
});
