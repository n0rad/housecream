...
var influx_query_1 = require("../influx_query");
common_1.describe('InfluxQuery', function () {
    ...
    common_1.describe('render series with mesurement only', function () {
        common_1.it('should generate correct query', function () {
            ...
            common_1.expect(queryText).to.be('SELECT mean("value") FROM "cpu" WHERE $timeFilter GROUP BY time($__interval) fill(null)');
        });
    });
    common_1.describe('render series with policy only', function () {
        common_1.it('should generate correct query', function () {
            ...
            var queryText = query.render();
            common_1.expect(queryText).to.be('SELECT mean("value") FROM "5m_avg"."cpu" WHERE $timeFilter GROUP BY time($__interval) fill(null)');
        });
    });
    common_1.describe('render series with math and alias', function () {
        common_1.it('should generate correct query', function () {
            ...
            var queryText = query.render();
            common_1.expect(queryText).to.be('SELECT mean("value") /100 AS "text" FROM "cpu" WHERE $timeFilter GROUP BY time($__interval) fill(null)');
        });
    });
    common_1.describe('series with single tag only', function () {
        ...
        common_1.it('should switch regex operator with tag value is regex', function () {
            ...
            var queryText = query.render();
            common_1.expect(queryText).to.be('SELECT mean("value") FROM "cpu" WHERE "app" =~ /e.*/ AND $timeFilter GROUP BY time($__interval)');
        });
    });
    ...
    common_1.describe('render series without group by and fill', function () {
        common_1.it('should generate correct query', function () {
            ...
            var queryText = query.render();
            common_1.expect(queryText).to.be('SELECT "value" FROM "cpu" WHERE $timeFilter GROUP BY time($__interval) fill(0)');
        });
    });
