...
var elastic_response_1 = require("../elastic_response");
common_1.describe('ElasticResponse', function () {
    ...
    common_1.describe('single group by with alias pattern', function () {
        ...
        common_1.it('should return 2 series', function () {
            common_1.expect(result.data.length).to.be(3);
            common_1.expect(result.data[0].datapoints.length).to.be(2);
            common_1.expect(result.data[0].target).to.be('server1 Count and {{not_exist}} server1');
            common_1.expect(result.data[1].target).to.be('server2 Count and {{not_exist}} server2');
            common_1.expect(result.data[2].target).to.be('0 Count and {{not_exist}} 0');
        });
    });
