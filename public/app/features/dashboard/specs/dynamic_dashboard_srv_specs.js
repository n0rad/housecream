...
dynamicDashScenario('given dashboard with panel repeat', function (ctx) {
    ...
    common_1.describe('After a second iteration with different variable', function () {
        common_1.beforeEach(function () {
        ...
        common_1.it('should remove scopedVars value for last variable', function () {
            common_1.expect(ctx.rows[0].panels[0].scopedVars.apps).to.be(undefined);
        });
        common_1.it('should have new variable value in scopedVars', function () {
            common_1.expect(ctx.rows[0].panels[0].scopedVars.server.value).to.be("se1");
        });
    });
    ...
    common_1.describe('After a second iteration and panel repeat is turned off', function () {
        ...
        common_1.it('should remove scoped vars from reused panel', function () {
            common_1.expect(ctx.rows[0].panels[0].scopedVars).to.be(undefined);
        });
    });
});
dynamicDashScenario('given dashboard with row repeat', function (ctx) {
    ...
    common_1.it('should generate a repeartRowId based on repeat row index', function () {
        ...
        common_1.expect(ctx.rows[1].repeatIteration).to.be(ctx.dynamicDashboardSrv.iteration);
    });
